...
import { useCompanyVerificationListStore } from "../stores/companyVerificationListStore"
import { postApiCompanyVerificationSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCompanyVerificationList = () => {
    const { t } = useTranslation()
    const store = useCompanyVerificationListStore()
    ...
    const query = useQuery({
        ...postApiCompanyVerificationSearchOptions({
            body: requestBody,
        }),
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("companyVerification.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const companyVerifications = query.data?.data?.items || []
