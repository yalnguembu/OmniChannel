...
import { postApiTemplateSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
...
export const useTemplateList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("template.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const templates = query.data?.data?.items || []
