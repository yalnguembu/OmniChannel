...
import { CreateTemplateRequest } from "@/shared/api"
import { zCreateTemplateRequest } from "@/shared/api/zod.gen"
...
interface TemplateCreateFormProps {
  onSubmit: (data: CreateTemplateRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateTemplateRequest>
...
export const TemplateCreateForm: React.FC<TemplateCreateFormProps> = ({ 
  ...
  const form = useForm<CreateTemplateRequest>({
    resolver: zodResolver(zCreateTemplateRequest),
    defaultValues: {
      ...defaultValues,
  ...
  const handleSubmit = (values: CreateTemplateRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("template.form.create.title")}</CardTitle>
        <CardDescription>{t("template.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
