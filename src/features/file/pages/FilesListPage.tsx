...
import { SearchFileRequest } from "@/shared/api/types.gen"
import { zSearchFileRequest } from "@/shared/api/zod.gen"
...
export function FilesListPage() {
  ...
  const { deleteMutation, bulkDeleteMutation } = useFileMutations()

  const handleCreate = () => {
    navigate({ to: `/file/add` })
  }
  ...
  return (
    <StandardListPageLayout
      header={
        <ListPageHeader
          title={t("file.title")}
          addButtonText={t("file.actions.add")}
          breadcrumbs={[{ label: t("navigation.dashboard"), href: "/dashboard" }, { label: t("file.title") }]}
          onCreate={handleCreate}
        />
      }
      filter={
        <BaseFilter<SearchFileRequest>
          schema={zSearchFileRequest}
