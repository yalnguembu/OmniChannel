...
import { CreateProductRequest } from "@/shared/api"
import { zCreateProductRequest } from "@/shared/api/zod.gen"
...
interface ProductCreateFormProps {
  onSubmit: (data: CreateProductRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
...
export const ProductCreateForm: React.FC<ProductCreateFormProps> = ({ 
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("product.form.create.title")}</CardTitle>
        <CardDescription>{t("product.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
