...
import { EditWebhookEndpointRequest } from "@/shared/api"
import { zEditWebhookEndpointRequest } from "@/shared/api/zod.gen"
...
interface WebhookEndpointEditFormProps {
  onSubmit: (data: EditWebhookEndpointRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditWebhookEndpointRequest>
  initialData?: Partial<EditWebhookEndpointRequest>
}

export const WebhookEndpointEditForm: React.FC<WebhookEndpointEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditWebhookEndpointRequest>({
    resolver: zodResolver(zEditWebhookEndpointRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditWebhookEndpointRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("webhookEndpoint.form.edit.title")}</CardTitle>
        <CardDescription>{t("webhookEndpoint.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("webhookEndpoint.form.edit.submit")}
              </Button>
            </div>
          </form>
        </Form>
