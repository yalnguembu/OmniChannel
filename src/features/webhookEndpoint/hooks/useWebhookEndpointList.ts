...
import { useWebhookEndpointListStore } from "../stores/webhookEndpointListStore"
import { postApiWebhookEndpointSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useWebhookEndpointList = () => {
    const { t } = useTranslation()
    const store = useWebhookEndpointListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("webhookEndpoint.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const webhookEndpoints = query.data?.data?.items || []
