...
export function WithdrawalsReadModelDetailsPage() {
  const navigate = useNavigate()
  const { id } = useParams({ from: "/_protected/transactions/withdrawals/$id/" })
  const { selectedWithdrawalsReadModel: data, getWithdrawalsReadModelQuery, isLoading, isError } = useWithdrawalsReadModel()

  const handleEdit = () => {
    navigate({ to: `/transactions/withdrawals/${id}/edit` })
  }
  ...
  useEffect(() => {
    if (id) {
      getWithdrawalsReadModelQuery(id)
    }
  }, [])
  ...
  if (isError || !data) {
    return (
      <div className="container mx-auto py-6 text-center">
        <p>WithdrawalsReadModel not found.</p>
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={handleEdit}>
          ...
          Edit WithdrawalsReadModel
        </Button>
      </div>

      <Card className="max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>WithdrawalsReadModel Details</CardTitle>
        </CardHeader>
