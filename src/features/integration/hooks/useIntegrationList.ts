...
import { postApiIntegrationSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useIntegrationList = () => {
    const { t } = useTranslation()
    const store = useIntegrationListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("integration.messages.search.error"))
        }
