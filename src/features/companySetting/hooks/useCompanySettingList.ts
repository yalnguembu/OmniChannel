...
import { postApiCompanySettingSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
...
export const useCompanySettingList = () => {
    ...
    const query = useQuery({
        ...postApiCompanySettingSearchOptions({
            body: requestBody,
        }),
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("companySetting.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const companySettings = query.data?.data?.items || []
