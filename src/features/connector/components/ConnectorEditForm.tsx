...
import { EditConnectorRequest } from "@/shared/api"
import { zEditConnectorRequest } from "@/shared/api/zod.gen"
...
interface ConnectorEditFormProps {
  onSubmit: (data: EditConnectorRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditConnectorRequest>
  initialData?: Partial<EditConnectorRequest>
}

export const ConnectorEditForm: React.FC<ConnectorEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditConnectorRequest>({
    resolver: zodResolver(zEditConnectorRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
...
const { data: companyDropdownData, isLoading: isCompanyLoading } = getCompanyOptionsQuery()
  ...
  const handleSubmit = (values: EditConnectorRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("connector.form.edit.title")}</CardTitle>
        <CardDescription>{t("connector.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
