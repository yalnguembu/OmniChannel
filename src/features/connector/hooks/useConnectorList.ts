...
import { useConnectorListStore } from "../stores/connectorListStore"
import { postApiConnectorSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useConnectorList = () => {
    const { t } = useTranslation()
    const store = useConnectorListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("connector.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const connectors = query.data?.data?.items || []
