...
import { useWalletTransactionListStore } from "../stores/walletTransactionListStore"
import { postApiWalletTransactionSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useWalletTransactionList = () => {
    const { t } = useTranslation()
    const store = useWalletTransactionListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("walletTransaction.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const walletTransactions = query.data?.data?.items || []
