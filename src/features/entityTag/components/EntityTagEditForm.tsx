...
import { EditEntityTagRequest } from "@/shared/api"
import { zEditEntityTagRequest } from "@/shared/api/zod.gen"
...
interface EntityTagEditFormProps {
  onSubmit: (data: EditEntityTagRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditEntityTagRequest>
  initialData?: Partial<EditEntityTagRequest>
  style?: string
}

export const EntityTagEditForm: React.FC<EntityTagEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditEntityTagRequest>({
    resolver: zodResolver(zEditEntityTagRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
...
const { data: tagDropdownData, isLoading: isTagLoading } = getTagOptionsQuery()
  ...
  const handleSubmit = (values: EditEntityTagRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("entityTag.form.edit.title")}</CardTitle>
        <CardDescription>{t("entityTag.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
