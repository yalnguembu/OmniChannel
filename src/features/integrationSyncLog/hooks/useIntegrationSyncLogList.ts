...
import { useIntegrationSyncLogListStore } from "../stores/integrationSyncLogListStore"
import { postApiIntegrationSyncLogSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useIntegrationSyncLogList = () => {
    const { t } = useTranslation()
    const store = useIntegrationSyncLogListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("integrationSyncLog.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const integrationSyncLogs = query.data?.data?.items || []
