...
import { CreateProviderCallbackRequest } from "@/shared/api"
import { zCreateProviderCallbackRequest } from "@/shared/api/zod.gen"
...
interface ProviderCallbackCreateFormProps {
  onSubmit: (data: CreateProviderCallbackRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateProviderCallbackRequest>
  
  style?: string
}

export const ProviderCallbackCreateForm: React.FC<ProviderCallbackCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateProviderCallbackRequest>({
    resolver: zodResolver(zCreateProviderCallbackRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: externalmessageDropdownData, isLoading: isExternalMessageLoading } = getExternalMessageOptionsQuery()
  ...
  const handleSubmit = (values: CreateProviderCallbackRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("providerCallback.form.create.title")}</CardTitle>
        <CardDescription>{t("providerCallback.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("providerCallback.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
