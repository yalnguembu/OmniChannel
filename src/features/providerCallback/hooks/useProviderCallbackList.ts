...
import { postApiProviderCallbackSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useProviderCallbackList = () => {
    ...
    const query = useQuery({
        ...postApiProviderCallbackSearchOptions({
            body: requestBody,
        }),
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("providerCallback.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const providerCallbacks = query.data?.data?.items || []
