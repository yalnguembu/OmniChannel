...
import { useSysLogListStore } from "../stores/sysLogListStore"
import { postApiSysLogSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useSysLogList = () => {
    const { t } = useTranslation()
    const store = useSysLogListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("sysLog.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const sysLogs = query.data?.data?.items || []
