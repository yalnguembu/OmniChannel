...
export function CompanyAppLimitDetailsPage() {
  const navigate = useNavigate()
  const { id } = useParams({ from: "/_protected/administration/company-app-limits/$id/" })
  const { selectedCompanyAppLimit: data, getCompanyAppLimitQuery, isLoading, isError } = useCompanyAppLimit()
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={handleEdit}>
          ...
          Edit CompanyAppLimit
        </Button>
      </div>

      <Card className="max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>CompanyAppLimit Details</CardTitle>
        </CardHeader>
