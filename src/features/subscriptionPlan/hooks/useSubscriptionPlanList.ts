...
import { useSubscriptionPlanListStore } from "../stores/subscriptionPlanListStore"
import { postApiSubscriptionPlanSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useSubscriptionPlanList = () => {
    const { t } = useTranslation()
    const store = useSubscriptionPlanListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("subscriptionPlan.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const subscriptionPlans = query.data?.data?.items || []
