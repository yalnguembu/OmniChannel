...
import { useWebhookDeliveryListStore } from "../stores/webhookDeliveryListStore"
import { postApiWebhookDeliverySearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useWebhookDeliveryList = () => {
    const { t } = useTranslation()
    const store = useWebhookDeliveryListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("webhookDelivery.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const webhookDeliverys = query.data?.data?.items || []
