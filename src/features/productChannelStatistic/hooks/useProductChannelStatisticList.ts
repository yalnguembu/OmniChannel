...
import { useProductChannelStatisticListStore } from "../stores/productChannelStatisticListStore"
import { postApiProductChannelStatisticSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useProductChannelStatisticList = () => {
    const { t } = useTranslation()
    const store = useProductChannelStatisticListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("productChannelStatistic.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const productChannelStatistics = query.data?.data?.items || []
