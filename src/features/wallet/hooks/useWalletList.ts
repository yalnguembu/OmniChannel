...
import { useWalletListStore } from "../stores/walletListStore"
import { postApiWalletSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useWalletList = () => {
    const { t } = useTranslation()
    const store = useWalletListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("wallet.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const wallets = query.data?.data?.items || []
