...
import { EditCurrencyRequest } from "@/shared/api"
import { zEditCurrencyRequest } from "@/shared/api/zod.gen"



interface CurrencyEditFormProps {
  onSubmit: (data: EditCurrencyRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditCurrencyRequest>
  initialData?: Partial<EditCurrencyRequest>
}
...
export const CurrencyEditForm: React.FC<CurrencyEditFormProps> = ({ 
  ...
  const form = useForm<EditCurrencyRequest>({
    resolver: zodResolver(zEditCurrencyRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditCurrencyRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("currency.form.edit.title")}</CardTitle>
        <CardDescription>{t("currency.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
