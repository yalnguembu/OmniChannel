...
import { useCurrencyListStore } from "../stores/currencyListStore"
import { postApiCurrencySearchOptions } from "@/shared/api/@tanstack/react-query.gen"
...
export const useCurrencyList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("currency.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const currencys = query.data?.data?.items || []
