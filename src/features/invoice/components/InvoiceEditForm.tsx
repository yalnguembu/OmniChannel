...
import { EditInvoiceRequest } from "@/shared/api"
import { zEditInvoiceRequest } from "@/shared/api/zod.gen"
...
interface InvoiceEditFormProps {
  onSubmit: (data: EditInvoiceRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditInvoiceRequest>
  initialData?: Partial<EditInvoiceRequest>
}
...
export const InvoiceEditForm: React.FC<InvoiceEditFormProps> = ({ 
  ...
  const form = useForm<EditInvoiceRequest>({
    resolver: zodResolver(zEditInvoiceRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditInvoiceRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("invoice.form.edit.title")}</CardTitle>
        <CardDescription>{t("invoice.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
