...
import { CreatePaymentRequest } from "@/shared/api"
import { zCreatePaymentRequest } from "@/shared/api/zod.gen"
...
interface PaymentCreateFormProps {
  onSubmit: (data: CreatePaymentRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreatePaymentRequest>
...
export const PaymentCreateForm: React.FC<PaymentCreateFormProps> = ({ 
  ...
  const form = useForm<CreatePaymentRequest>({
    resolver: zodResolver(zCreatePaymentRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: finaltransactionDropdownData, isLoading: isFinalTransactionLoading } = getFinalTransactionOptionsQuery()
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("payment.form.create.title")}</CardTitle>
        <CardDescription>{t("payment.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
