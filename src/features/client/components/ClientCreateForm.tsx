...
import { CreateClientRequest } from "@/shared/api"
import { zCreateClientRequest } from "@/shared/api/zod.gen"
...
interface ClientCreateFormProps {
  onSubmit: (data: CreateClientRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
...
export const ClientCreateForm: React.FC<ClientCreateFormProps> = ({ 
  ...
  const form = useForm<CreateClientRequest>({
    resolver: zodResolver(zCreateClientRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: externalDropdownData, isLoading: isExternalLoading } = getExternalOptionsQuery()
  ...
  const handleSubmit = (values: CreateClientRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("client.form.create.title")}</CardTitle>
        <CardDescription>{t("client.form.create.description")}</CardDescription>
