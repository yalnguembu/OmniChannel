...
import { useMessageEventListStore } from "../stores/messageEventListStore"
import { postApiMessageEventSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useMessageEventList = () => {
    const { t } = useTranslation()
    const store = useMessageEventListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("messageEvent.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const messageEvents = query.data?.data?.items || []
