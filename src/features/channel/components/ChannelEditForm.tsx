...
import { EditChannelRequest } from "@/shared/api"
import { zEditChannelRequest } from "@/shared/api/zod.gen"



interface ChannelEditFormProps {
  onSubmit: (data: EditChannelRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditChannelRequest>
  initialData?: Partial<EditChannelRequest>
}
...
export const ChannelEditForm: React.FC<ChannelEditFormProps> = ({ 
  ...
  const form = useForm<EditChannelRequest>({
    resolver: zodResolver(zEditChannelRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditChannelRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("channel.form.edit.title")}</CardTitle>
        <CardDescription>{t("channel.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
