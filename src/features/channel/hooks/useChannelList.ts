...
import { postApiChannelSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useChannelList = () => {
    const { t } = useTranslation()
    const store = useChannelListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("channel.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const channels = query.data?.data?.items || []
