...
export const useCompanyMutations = () => {
    ...
    const createCompanyMutation = useMutation({
        ...
        onError: (error) => {
            // Default error handling if not handled by createCompanyWithValidation
            if (!error) return; // Handled elsewhere potentially
        }
    })
    ...
    const bulkDeleteMutation = useMutation({
        ...
        onError: () =>
            toast.error(t("companies.bulk.deleteError"))
    })
