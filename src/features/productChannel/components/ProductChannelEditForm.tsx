...
import { EditProductChannelRequest } from "@/shared/api"
import { zEditProductChannelRequest } from "@/shared/api/zod.gen"
...
interface ProductChannelEditFormProps {
  onSubmit: (data: EditProductChannelRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditProductChannelRequest>
  initialData?: Partial<EditProductChannelRequest>
  style?: string
}

export const ProductChannelEditForm: React.FC<ProductChannelEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditProductChannelRequest>({
    resolver: zodResolver(zEditProductChannelRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
...
const { data: channelDropdownData, isLoading: isChannelLoading } = getChannelOptionsQuery()
  ...
  const handleSubmit = (values: EditProductChannelRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("productChannel.form.edit.title")}</CardTitle>
        <CardDescription>{t("productChannel.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("productChannel.form.edit.submit")}
              </Button>
            </div>
          </form>
        </Form>
