...
import { postApiProductChannelSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useProductChannelList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("productChannel.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const productChannels = query.data?.data?.items || []
