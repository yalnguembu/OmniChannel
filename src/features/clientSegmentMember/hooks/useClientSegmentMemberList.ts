...
import { postApiClientSegmentMemberSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useClientSegmentMemberList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("clientSegmentMember.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const clientSegmentMembers = query.data?.data?.items || []
