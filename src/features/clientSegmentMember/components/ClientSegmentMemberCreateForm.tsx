...
import { CreateClientSegmentMemberRequest } from "@/shared/api"
import { zCreateClientSegmentMemberRequest } from "@/shared/api/zod.gen"
...
interface ClientSegmentMemberCreateFormProps {
  onSubmit: (data: CreateClientSegmentMemberRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateClientSegmentMemberRequest>
  
  style?: string
}

export const ClientSegmentMemberCreateForm: React.FC<ClientSegmentMemberCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateClientSegmentMemberRequest>({
    resolver: zodResolver(zCreateClientSegmentMemberRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: segmentDropdownData, isLoading: isSegmentLoading } = getSegmentOptionsQuery()
  ...
  const handleSubmit = (values: CreateClientSegmentMemberRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("clientSegmentMember.form.create.title")}</CardTitle>
        <CardDescription>{t("clientSegmentMember.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("clientSegmentMember.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
