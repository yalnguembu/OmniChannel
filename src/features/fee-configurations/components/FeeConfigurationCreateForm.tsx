...
import { zCreateFeeConfigurationRequest } from "@/shared/api/zod.gen"
...
export const FeeConfigurationCreateForm: React.FC<FeeConfigurationCreateFormProps> = ({ onSubmit, onCancel, isLoading = false, defaultValues, companyId, applicationId }) => {
  const { t } = useTranslation()
  const [ownerType, setOwnerType] = useState<OWNER_TYPE>(OWNER_TYPE.SYSTEM)

  const form = useForm<CreateFeeConfigurationRequest>({
    resolver: zodResolver(zCreateFeeConfigurationRequest),
    defaultValues: companyId
      ? { ...defaultValues, ownerType: OWNER_TYPE.COMPANY, ownerId: companyId }
      : applicationId
        ? { ...defaultValues, ownerType: OWNER_TYPE.APPLICATION, ownerId: applicationId }
        : {
            ...defaultValues,
          },
  })
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("feeConfigurations.form.create.title")}</CardTitle>
        <CardDescription>{t("feeConfigurations.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
