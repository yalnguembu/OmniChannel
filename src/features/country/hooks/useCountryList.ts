...
import { useCountryListStore } from "../stores/countryListStore"
import { postApiCountrySearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCountryList = () => {
    const { t } = useTranslation()
    const store = useCountryListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("country.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const countrys = query.data?.data?.items || []
