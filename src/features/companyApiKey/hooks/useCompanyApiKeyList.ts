...
import { postApiCompanyApiKeySearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCompanyApiKeyList = () => {
    ...
    const query = useQuery({
        ...postApiCompanyApiKeySearchOptions({
            body: requestBody,
        }),
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("companyApiKey.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const companyApiKeys = query.data?.data?.items || []
