...
import { useSettingListStore } from "../stores/settingListStore"
import { postApiSettingSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useSettingList = () => {
    const { t } = useTranslation()
    const store = useSettingListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("setting.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const settings = query.data?.data?.items || []
