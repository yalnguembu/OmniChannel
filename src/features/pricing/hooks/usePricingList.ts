...
import { usePricingListStore } from "../stores/pricingListStore"
import { postApiPricingSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const usePricingList = () => {
    const { t } = useTranslation()
    const store = usePricingListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("pricing.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const pricings = query.data?.data?.items || []
