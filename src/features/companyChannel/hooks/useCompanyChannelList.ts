...
import { postApiCompanyChannelSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCompanyChannelList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("companyChannel.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const companyChannels = query.data?.data?.items || []
