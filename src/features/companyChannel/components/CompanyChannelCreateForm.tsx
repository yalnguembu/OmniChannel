...
import { CreateCompanyChannelRequest } from "@/shared/api"
import { zCreateCompanyChannelRequest } from "@/shared/api/zod.gen"
...
interface CompanyChannelCreateFormProps {
  onSubmit: (data: CreateCompanyChannelRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateCompanyChannelRequest>
  
  style?: string
}

export const CompanyChannelCreateForm: React.FC<CompanyChannelCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateCompanyChannelRequest>({
    resolver: zodResolver(zCreateCompanyChannelRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: channelDropdownData, isLoading: isChannelLoading } = getChannelOptionsQuery()
  ...
  const handleSubmit = (values: CreateCompanyChannelRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("companyChannel.form.create.title")}</CardTitle>
        <CardDescription>{t("companyChannel.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("companyChannel.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
