...
import { usePaymentMethodListStore } from "../stores/paymentMethodListStore"
import { postApiPaymentMethodSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const usePaymentMethodList = () => {
    const { t } = useTranslation()
    const store = usePaymentMethodListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("paymentMethod.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const paymentMethods = query.data?.data?.items || []
