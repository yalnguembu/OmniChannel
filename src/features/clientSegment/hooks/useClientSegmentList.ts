...
import { postApiClientSegmentSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useClientSegmentList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("clientSegment.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const clientSegments = query.data?.data?.items || []
