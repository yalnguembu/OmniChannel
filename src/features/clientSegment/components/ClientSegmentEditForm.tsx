...
import { EditClientSegmentRequest } from "@/shared/api"
import { zEditClientSegmentRequest } from "@/shared/api/zod.gen"
...
interface ClientSegmentEditFormProps {
  onSubmit: (data: EditClientSegmentRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditClientSegmentRequest>
  initialData?: Partial<EditClientSegmentRequest>
  style?: string
}

export const ClientSegmentEditForm: React.FC<ClientSegmentEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditClientSegmentRequest>({
    resolver: zodResolver(zEditClientSegmentRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditClientSegmentRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("clientSegment.form.edit.title")}</CardTitle>
        <CardDescription>{t("clientSegment.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("clientSegment.form.edit.submit")}
              </Button>
            </div>
          </form>
        </Form>
