...
import { CreateCampaignChannelRequest } from "@/shared/api"
import { zCreateCampaignChannelRequest } from "@/shared/api/zod.gen"
...
interface CampaignChannelCreateFormProps {
  onSubmit: (data: CreateCampaignChannelRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateCampaignChannelRequest>
  
  style?: string
}

export const CampaignChannelCreateForm: React.FC<CampaignChannelCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateCampaignChannelRequest>({
    resolver: zodResolver(zCreateCampaignChannelRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: templateDropdownData, isLoading: isTemplateLoading } = getTemplateOptionsQuery()
  ...
  const handleSubmit = (values: CreateCampaignChannelRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("campaignChannel.form.create.title")}</CardTitle>
        <CardDescription>{t("campaignChannel.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("campaignChannel.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
