...
import { postApiCampaignChannelSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCampaignChannelList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("campaignChannel.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const campaignChannels = query.data?.data?.items || []
