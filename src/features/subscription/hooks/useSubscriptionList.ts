...
import { useSubscriptionListStore } from "../stores/subscriptionListStore"
import { postApiSubscriptionSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useSubscriptionList = () => {
    const { t } = useTranslation()
    const store = useSubscriptionListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("subscription.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const subscriptions = query.data?.data?.items || []
