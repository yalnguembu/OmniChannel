...
import { CreateSubscriptionRequest } from "@/shared/api"
import { zCreateSubscriptionRequest } from "@/shared/api/zod.gen"
...
interface SubscriptionCreateFormProps {
  onSubmit: (data: CreateSubscriptionRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
...
export const SubscriptionCreateForm: React.FC<SubscriptionCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateSubscriptionRequest>({
    resolver: zodResolver(zCreateSubscriptionRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: paymentDropdownData, isLoading: isPaymentLoading } = getPaymentOptionsQuery()
  ...
  const handleSubmit = (values: CreateSubscriptionRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("subscription.form.create.title")}</CardTitle>
        <CardDescription>{t("subscription.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("subscription.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
