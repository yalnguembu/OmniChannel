...
import { EditClientImportRequest } from "@/shared/api"
import { zEditClientImportRequest } from "@/shared/api/zod.gen"
...
interface ClientImportEditFormProps {
  onSubmit: (data: EditClientImportRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditClientImportRequest>
  initialData?: Partial<EditClientImportRequest>
}

export const ClientImportEditForm: React.FC<ClientImportEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditClientImportRequest>({
    resolver: zodResolver(zEditClientImportRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditClientImportRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("clientImport.form.edit.title")}</CardTitle>
        <CardDescription>{t("clientImport.form.edit.description")}</CardDescription>
              ...
              <div className="flex space-x-4">
                ...
                {currentStep < totalSteps - 1 ? (
                  ...
                  <Button type="submit" disabled={isLoading}>
                    ...
                    {t("clientImport.form.edit.submit")}
                  </Button>
                )}
              </div>
            </div>
