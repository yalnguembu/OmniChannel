...
import { useClientImportListStore } from "../stores/clientImportListStore"
import { postApiClientImportSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useClientImportList = () => {
    const { t } = useTranslation()
    const store = useClientImportListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("clientImport.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const clientImports = query.data?.data?.items || []
