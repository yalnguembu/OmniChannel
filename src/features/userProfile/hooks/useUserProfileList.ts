...
import { useUserProfileListStore } from "../stores/userProfileListStore"
import { postApiUserProfileSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useUserProfileList = () => {
    const { t } = useTranslation()
    const store = useUserProfileListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("userProfile.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const userProfiles = query.data?.data?.items || []
