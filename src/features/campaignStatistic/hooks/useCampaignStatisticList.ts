...
import { useCampaignStatisticListStore } from "../stores/campaignStatisticListStore"
import { postApiCampaignStatisticSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCampaignStatisticList = () => {
    const { t } = useTranslation()
    const store = useCampaignStatisticListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("campaignStatistic.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const campaignStatistics = query.data?.data?.items || []
