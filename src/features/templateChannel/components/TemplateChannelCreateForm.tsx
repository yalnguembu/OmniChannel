...
import { CreateTemplateChannelRequest } from "@/shared/api"
import { zCreateTemplateChannelRequest } from "@/shared/api/zod.gen"
...
interface TemplateChannelCreateFormProps {
  onSubmit: (data: CreateTemplateChannelRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateTemplateChannelRequest>
  
  style?: string
}

export const TemplateChannelCreateForm: React.FC<TemplateChannelCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateTemplateChannelRequest>({
    resolver: zodResolver(zCreateTemplateChannelRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: channelDropdownData, isLoading: isChannelLoading } = getChannelOptionsQuery()
  ...
  const handleSubmit = (values: CreateTemplateChannelRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("templateChannel.form.create.title")}</CardTitle>
        <CardDescription>{t("templateChannel.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("templateChannel.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
