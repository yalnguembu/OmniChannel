...
import { postApiTemplateChannelSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useTemplateChannelList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("templateChannel.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const templateChannels = query.data?.data?.items || []
