...
export const ApplicationEditForm: React.FC<ApplicationEditFormProps> = ({ applicationId, initialData, onSubmit, onCancel, isLoading = false, companyId, style }) => {
  const { t } = useTranslation()

  const { getCompanyOptionsQuery } = useApplicationOptions()
  ...
  const handleSubmit = (values: UpdateApplicationRequest) => {
    if (onSubmit) {
      onSubmit({ ...values, id: applicationId || initialData.id }, form.setError)
    }
  }
