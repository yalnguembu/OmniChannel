...
import { Edit, ArrowLeft, Loader2, Building, Key, EyeClosed, Eye, Calendar, RotateCcwKey, Copy } from "lucide-react"
...
export function ApplicationDetailsPage() {
  ...
  const { application, apiKey, isLoading, isError } = useApplicationDetail(id)
  const { regenerateApiSecret } = useApplicationMutations()
  ...
  const companyDetails = application
          ...
          <div className="grid lg:grid-cols-5 gap-4">
                ...
                <div className="flex flex-col gap-y-1">
                  ...
                  <CardDescription className="text-blue-400 hover:text-blue-600 flex gap-x-2">
                    ...
                    <Link to={`/companies/${companyDetails.companyId || ""}/` as `/companies`} className="link hover:underline" target="_blank" rel="noopener noreferrer">
                      {companyDetails.name || "https://templateweb.com"}
                    </Link>
                  </CardDescription>
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <CardAction>
                  <Button variant="outline" size="sm" className="h-8 rounded-lg" disabled={isLoading} onClick={() => regenerateApiSecret(id)}>
                    {isLoading ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <RotateCcwKey className="h-4 w-4 mr-2" />}
                    Regenerate
                  </Button>
                </CardAction>
              </CardHeader>
              <CardContent>
                <ApiKeyDetailItem label="Key:" value={apiKey?.apiKey || "-"} />
                <ApiKeyDetailItem label="Secret:" value={apiKey?.apiSecret || "-"} />
                ...
                <div className=" text-xs flex items-center mt-2">
                  ...
                  <span className="ml-2">{apiKey?.apiKeyExpiresAtUtc ? formatDate(apiKey.apiKeyExpiresAtUtc, DateFormat.DATETIME_SHORT) : "-"}</span>
                </div>
              </CardContent>
            </Card>
          </div>
