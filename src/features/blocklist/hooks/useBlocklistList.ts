...
import { useBlocklistListStore } from "../stores/blocklistListStore"
import { postApiBlocklistSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useBlocklistList = () => {
    const { t } = useTranslation()
    const store = useBlocklistListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("blocklist.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const blocklists = query.data?.data?.items || []
