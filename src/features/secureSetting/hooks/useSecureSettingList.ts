...
import { useSecureSettingListStore } from "../stores/secureSettingListStore"
import { postApiSecureSettingSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useSecureSettingList = () => {
    const { t } = useTranslation()
    const store = useSecureSettingListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("secureSetting.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const secureSettings = query.data?.data?.items || []
