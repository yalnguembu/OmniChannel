...
import { postApiMessageSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
...
export const useMessageList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("message.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const messages = query.data?.data?.items || []
