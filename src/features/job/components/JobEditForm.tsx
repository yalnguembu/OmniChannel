...
import { EditJobRequest } from "@/shared/api"
import { zEditJobRequest } from "@/shared/api/zod.gen"



interface JobEditFormProps {
  onSubmit: (data: EditJobRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditJobRequest>
  initialData?: Partial<EditJobRequest>
}
...
export const JobEditForm: React.FC<JobEditFormProps> = ({ 
  ...
  const form = useForm<EditJobRequest>({
    resolver: zodResolver(zEditJobRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
  ...
  const handleSubmit = (values: EditJobRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{t("job.form.edit.title")}</CardTitle>
        <CardDescription>{t("job.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
