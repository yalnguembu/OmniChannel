...
import { postApiClientChannelPreferenceSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useClientChannelPreferenceList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("clientChannelPreference.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const clientChannelPreferences = query.data?.data?.items || []
