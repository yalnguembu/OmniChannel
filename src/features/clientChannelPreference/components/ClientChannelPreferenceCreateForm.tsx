...
import { CreateClientChannelPreferenceRequest } from "@/shared/api"
import { zCreateClientChannelPreferenceRequest } from "@/shared/api/zod.gen"
...
interface ClientChannelPreferenceCreateFormProps {
  onSubmit: (data: CreateClientChannelPreferenceRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<CreateClientChannelPreferenceRequest>
  
  style?: string
}

export const ClientChannelPreferenceCreateForm: React.FC<ClientChannelPreferenceCreateFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<CreateClientChannelPreferenceRequest>({
    resolver: zodResolver(zCreateClientChannelPreferenceRequest),
    defaultValues: {
      ...defaultValues,
...
const { data: channelDropdownData, isLoading: isChannelLoading } = getChannelOptionsQuery()
  ...
  const handleSubmit = (values: CreateClientChannelPreferenceRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("clientChannelPreference.form.create.title")}</CardTitle>
        <CardDescription>{t("clientChannelPreference.form.create.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("clientChannelPreference.form.create.submit")}
              </Button>
            </div>
          </form>
        </Form>
