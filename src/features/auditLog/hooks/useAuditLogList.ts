...
import { useAuditLogListStore } from "../stores/auditLogListStore"
import { postApiAuditLogSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useAuditLogList = () => {
    const { t } = useTranslation()
    const store = useAuditLogListStore()
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("auditLog.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const auditLogs = query.data?.data?.items || []
