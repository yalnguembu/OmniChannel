...
import { postApiCampaignSegmentSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
import { useEffect } from "react"
import { toast } from "sonner"

export const useCampaignSegmentList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("campaignSegment.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const campaignSegments = query.data?.data?.items || []
