...
import { EditCampaignSegmentRequest } from "@/shared/api"
import { zEditCampaignSegmentRequest } from "@/shared/api/zod.gen"
...
interface CampaignSegmentEditFormProps {
  onSubmit: (data: EditCampaignSegmentRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditCampaignSegmentRequest>
  initialData?: Partial<EditCampaignSegmentRequest>
  style?: string
}

export const CampaignSegmentEditForm: React.FC<CampaignSegmentEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditCampaignSegmentRequest>({
    resolver: zodResolver(zEditCampaignSegmentRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
...
const { data: segmentDropdownData, isLoading: isSegmentLoading } = getSegmentOptionsQuery()
  ...
  const handleSubmit = (values: EditCampaignSegmentRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("campaignSegment.form.edit.title")}</CardTitle>
        <CardDescription>{t("campaignSegment.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("campaignSegment.form.edit.submit")}
              </Button>
            </div>
          </form>
        </Form>
