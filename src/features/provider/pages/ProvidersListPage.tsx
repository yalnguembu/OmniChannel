...
import { ProviderDataGrid } from "../components/ProviderDataGrid"
...
import { zSearchProviderRequest } from "@/shared/api/zod.gen"
import { SearchProviderRequest, CreateProviderRequest, UpdateProviderRequest } from "@/shared/api/types.gen"
...
import { useProviderListStore } from "../stores/providerListStore"
...
export function ProvidersListPage() {
  ...
  const { createMutation, updateMutation, deleteMutation, bulkDeleteMutation } = useProviderMutations()
  const store = useProviderListStore()
  
  const { provider: selectedItem, isLoading: isDetailsLoading } = useProviderDetail(store.selectedItemId || "")
  ...
  const handleUpdate = (data: UpdateProviderRequest) => {
  ...
  return (
    <>
      <StandardListPageLayout
        header={
          <ListPageHeader
            ...
            addButtonText={t("provider.actions.add")}
            breadcrumbs={[{ label: t("navigation.dashboard"), href: "/dashboard" }, { label: t("provider.title") }]}
            onCreate={() => store.setShowCreateModal(true)}
          />
        }
        filter={
          <BaseFilter<SearchProviderRequest>
            schema={zSearchProviderRequest}
            ...
            fieldTranslationPrefix="provider"
          />
        }
        content={
          <ProviderDataGrid 
      ...
      <ModalWrapper
        ...
        withHeader
        title={t("provider.form.create.title")}
        size="lg"
      >
        <ProviderCreateForm
      ...
      <ModalWrapper
        ...
        withHeader
        title={t("provider.form.edit.title")}
        size="lg"
      >
      ...
      >
         {isDetailsLoading && <div className="p-4 flex justify-center">Loading...</div>}
        {selectedItem && !isDetailsLoading && (
          <ProviderDetails data={selectedItem} />
        )}
      </ModalWrapper>
      ...
      <ConfirmationModal
        ...
        description={t("provider.messages.delete.confirm", { name: selectedItem?.id || store.selectedItemId })}
