...
import { postApiCampaignStepSearchOptions } from "@/shared/api/@tanstack/react-query.gen"
...
export const useCampaignStepList = () => {
    ...
    useEffect(() => {
        if (query.isError) {
            toast.error(t("campaignStep.messages.search.error"))
        }
    }, [query.isError, query.error, t])

    const campaignSteps = query.data?.data?.items || []
