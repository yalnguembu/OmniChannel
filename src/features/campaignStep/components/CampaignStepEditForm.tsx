...
import { EditCampaignStepRequest } from "@/shared/api"
import { zEditCampaignStepRequest } from "@/shared/api/zod.gen"
...
interface CampaignStepEditFormProps {
  onSubmit: (data: EditCampaignStepRequest, setError: any) => void
  onCancel: () => void
  isLoading?: boolean
  defaultValues?: Partial<EditCampaignStepRequest>
  initialData?: Partial<EditCampaignStepRequest>
  style?: string
}

export const CampaignStepEditForm: React.FC<CampaignStepEditFormProps> = ({ 
  onSubmit, 
  onCancel, 
  isLoading = false, 
  defaultValues,
  ...
  const form = useForm<EditCampaignStepRequest>({
    resolver: zodResolver(zEditCampaignStepRequest),
    defaultValues: {
      ...defaultValues,
      ...initialData,
    },
...
const { data: channelDropdownData, isLoading: isChannelLoading } = getChannelOptionsQuery()
  ...
  const handleSubmit = (values: EditCampaignStepRequest) => {
    if (onSubmit) {
  ...
  return (
    <Card className={`w-full max-w-4xl mx-auto ${style}`}>
      <CardHeader>
        <CardTitle>{t("campaignStep.form.edit.title")}</CardTitle>
        <CardDescription>{t("campaignStep.form.edit.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
    ...
    />
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {t("campaignStep.form.edit.submit")}
              </Button>
            </div>
          </form>
        </Form>
