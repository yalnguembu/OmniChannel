...
[src/shared/api/setup.ts](src/shared/api/setup.ts) configures the generated axios client (baseURL from `VITE_API_URL`, injects the Bearer token from `authStore`) and exports `handleRequest<T>()`, which normalizes any SDK call into `ApiResponse<T>` = `{ success: true, data }` | `FailedResponse`. The old hand-rolled domain service wrappers (`src/shared/api/services/*.ts`, e.g. `ProductService`) have been **removed** — consume the generated TanStack Query helpers directly via a `use*ViewModel` (the MVVM pattern), or call the generated `sdk.gen.ts` functions directly for the rare multipart/branching cases (e.g. `postApiClientImportUpload`, the multipart `/api/File` upload). Password endpoints aren't in the spec, so [src/shared/api/authPassword.ts](src/shared/api/authPassword.ts) still hits `/api/auth/*` directly via the configured client.
...
- **WhatsApp inbox** — calls the generated `sdk.gen.ts` functions **directly** (not the TanStack Query `...Options`/`...Mutation` helpers) from its own React Query hooks ([useWhatsapp.ts](src/hooks/useWhatsapp.ts)); the media/host base URL lives in [src/shared/api/whatsappBaseUrl.ts](src/shared/api/whatsappBaseUrl.ts) (derived from `VITE_API_URL`, not the old hardcoded host). It keeps its own store ([useWhatsappStore.ts](src/store/useWhatsappStore.ts)), models ([whatsapp.models.ts](src/models/whatsapp.models.ts)), and real-time updates over SignalR ([useSignalR.ts](src/hooks/useSignalR.ts), hub at `/hubs/conversations`).
